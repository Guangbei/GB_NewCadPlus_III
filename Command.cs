...
namespace GB_NewCadPlus_III
{
    ...
    public class Command
    {
        ...
        /// 从边界字符串计算中心点（辅助方法）
        /// </summary>
        /// <param name="bounds">边界字符串（格式："minX,minY,minZ;maxX,maxY,maxZ"）</param>
        /// <returns>边界中心点</returns>
        private static Point3d GetBoundsCenter(string bounds)
        {
            try
            {
                // 分割边界范围（格式："minX,minY,minZ;maxX,maxY,maxZ"）
                string[] parts = bounds.Split(';');
                if (parts.Length != 2) throw new FormatException("边界格式错误");

                // 解析最小坐标
                string[] minCoords = parts[0].Split(',');
                if (minCoords.Length != 3) throw new FormatException("边界格式错误");
                double minX = double.Parse(minCoords[0]);
                double minY = double.Parse(minCoords[1]);
                double minZ = double.Parse(minCoords[2]);

                // 解析最大坐标
                string[] maxCoords = parts[1].Split(',');
                if (maxCoords.Length != 3) throw new FormatException("边界格式错误");
                double maxX = double.Parse(maxCoords[0]);
                double maxY = double.Parse(maxCoords[1]);
                double maxZ = double.Parse(maxCoords[2]);
            ...
            {
                Application.ShowAlertDialog($"计算边界中心点失败：{ex.Message}，将使用原点(0,0,0)插入。");
                return Point3d.Origin; // 默认使用原点
            }
        }
        ...
        /// 处理嵌套块引用(确保组合块结构完整)  
        /// </summary>  
        private static void ProcessNestedBlocks(DBTrans tr, BlockReference parentBlockRef)
        {
            // 获取块定义  
            BlockTableRecord btr = tr.GetObject(parentBlockRef.BlockTableRecord, OpenMode.ForRead) as BlockTableRecord;
            ...
            foreach (ObjectId entId in btr)
            {
                ...
                {
                    // 如果找到嵌套块引用，确保其属性和变换正确应用  
                    // 复合块的关键在于确保所有嵌套结构和属性正确处理  
                    if (nestedBlockRef.AttributeCollection != null && nestedBlockRef.AttributeCollection.Count > 0)
                    {
                        foreach (ObjectId attId in nestedBlockRef.AttributeCollection)
                        {
                            AttributeReference att = (AttributeReference)tr.GetObject(attId, OpenMode.ForRead);
                            // 可以在这里处理嵌套块的属性，如需要  
                        }
                    ...
                    // 递归处理更深层次的嵌套块  
                    ProcessNestedBlocks(tr, nestedBlockRef);
                }
            }
        }
        ...
        /// <param name="xrefBlockRef"></param>
        /// <param name="tr"></param>
        /// <returns></returns>
        private bool IsEntityInXref(Entity entity, BlockReference xrefBlockRef, Transaction tr)
        {
            // 获取图元的所有者
            ObjectId ownerId = entity.OwnerId;
            BlockTableRecord ownerBtr = tr.GetObject(ownerId, OpenMode.ForRead) as BlockTableRecord;

            if (ownerBtr == null)
                return false;

            // 检查块表记录的路径是否与外部参照的路径匹配
            if (ownerBtr.IsFromExternalReference)
            {
                // 获取外部参照的路径
                BlockTableRecord xrefBtr = tr.GetObject(xrefBlockRef.BlockTableRecord, OpenMode.ForRead) as BlockTableRecord;
                if (xrefBtr != null && xrefBtr.PathName == ownerBtr.PathName)
                    return true;
            }
            //// 检查嵌套情况
            //if (ownerBtr.IsAnonymous)
            //{

            //    // 匿名块可能是外部参照的一部分
            //    return IsEntityInXref(ownerBtr, xrefBlockRef, tr);
            //}
        ...
        /// <param name="VariableDictionary.layerColorIndex"></param>
        /// <returns></returns>
        private static ObjectId CreateMleaderStyle(string MleaderStyleName, Int16 layerColorIndex)
        {
            ...
            try
            {
                using (var acTrans = acDoc.TransactionManager.StartTransaction())
                {
                    ...
                    {
                        return DbDictionary.GetAt(MleaderStyleName);
                    }
                    ...
                    newMleadStyle.LeaderLineColor = Color.FromColorIndex(ColorMethod.ByAci, layerColorIndex);
                    ...
                    newMleadStyle.PostMLeaderStyleToDb(db, MleaderStyleName);
            ...
            {
                // 记录错误日志  
                Env.Editor.WriteMessage("创建M1样式失败！");
                Env.Editor.WriteMessage($"错误信息: {ex.Message}");
            }
        ...
        // 可调：采样点数，越大轮廓越圆润，但处理更慢，建议64或128  
        const int OutlineSampleNum = 128; // 定义用于生成轮廓的采样点数量  
        ...
        #region 方法二：这个方法可以用，只是把所有的图无框在里面，没有凹方法

        ///// <summary>  
        ///// 命令入口：选择图元生成外轮廓（向外扩10单位）  
        ///// </summary>  
        //[CommandMethod(nameof(CreateOutlineCommand))]
        //public void CreateOutlineCommand()
        //{
        //    // 提示用户选择图元（空格或右键确认选择由 AutoCAD 默认处理）  
        //    PromptSelectionOptions selOpts = new PromptSelectionOptions
        //    {
        //        MessageForAdding = "\n请选择生成外轮廓的图元，确认选择请按空格或右键："
        //    };

        //    PromptSelectionResult selRes = Env.Editor.GetSelection(selOpts);
        ...
        //        // 尝试创建外轮廓，使用凸包方法  
        //        Polyline finalOutline = null;

        //        finalOutline = CreateDirectOutline(tr, selRes.Value, 10.0);

        //        // 验证最终结果是否有效  
        //        if (finalOutline != null)
        //        {
        //            // 确保多段线是闭合的  
        //            finalOutline.Closed = true;

        //            // 设置颜色和其他属性  
        //            finalOutline.ColorIndex = 1; // 红色  
        //            finalOutline.ConstantWidth = 0;

        //            // 添加到当前空间  
        //            tr.CurrentSpace.AddEntity(finalOutline);
        //            tr.Commit();
        ...
        ///// </summary>
        ///// <param name="tr">事务</param>
        ///// <param name="selSet">选择图元</param>
        ///// <param name="offsetDist">偏移距离</param>
        ///// <returns></returns>
        //private Polyline CreateDirectOutline(DBTrans tr, SelectionSet selSet, double offsetDist)
        //{
        //    try
        //    {
        //        Env.Editor.WriteMessage("\n使用凸包方法创建外轮廓...");
        ...
        //            allPoints.Add(new Point2d(ext.MinPoint.X, ext.MinPoint.Y));
        //            allPoints.Add(new Point2d(ext.MaxPoint.X, ext.MinPoint.Y));
        //            allPoints.Add(new Point2d(ext.MaxPoint.X, ext.MaxPoint.Y));
        //            allPoints.Add(new Point2d(ext.MinPoint.X, ext.MaxPoint.Y));
        ...
        //                    allPoints.Add(new Point2d(center.X + radius, center.Y));
        //                    allPoints.Add(new Point2d(center.X, center.Y + radius));
        //                    allPoints.Add(new Point2d(center.X - radius, center.Y));
        //                    allPoints.Add(new Point2d(center.X, center.Y - radius));
        ...
        //                    {
        //                        allPoints.Add(polyline.GetPoint2dAt(i));
        //                    }
        //                }
        //                // 对于弧线，添加起点、中点和终点以及更多采样点  
        //                else if (ent is Arc arc)
        //                {
        //                    allPoints.Add(new Point2d(arc.StartPoint.X, arc.StartPoint.Y));
        //                    allPoints.Add(new Point2d(arc.EndPoint.X, arc.EndPoint.Y));
        ...
        //                            0);
        //                        allPoints.Add(new Point2d(pt.X, pt.Y));
        //                    }
        //                }
        //                // 对于线段，添加起点和终点  
        //                else if (ent is Line line)
        //                {
        //                    allPoints.Add(new Point2d(line.StartPoint.X, line.StartPoint.Y));
        //                    allPoints.Add(new Point2d(line.EndPoint.X, line.EndPoint.Y));
        //                }
        //            }
        ...
        //            {
        //                GetBlockReferencePoints(tr, blkRef, allPoints);
        //            }
        //        }
        ...
        //        // 2. 使用凸包算法创建包含所有点的多边形  
        //        List<Point2d> convexHull = ComputeConvexHull(allPoints);
        ...
        //        {
        //            hullPoly.AddVertexAt(i, convexHull[i], 0, 0, 0);
        //        }
        //        hullPoly.Closed = true;

        //        // 4. 偏移凸包多段线  
        //        DBObjectCollection offsetResults = hullPoly.GetOffsetCurves(offsetDist);
        //        if (offsetResults.Count == 0)
        //        {
        //            Env.Editor.WriteMessage("\n偏移操作失败。");
        //            return null;
        //        }

        //        Polyline offsetPoly = offsetResults[0] as Polyline;
        //        return offsetPoly;
        //    }
        //    catch (Exception ex)
        //    {
        //        Env.Editor.WriteMessage($"\n创建凸包轮廓时出错: {ex.Message}");
        //        return null;
        //    }
        //}
        ...
        ///// 递归获取块参照中所有图元的特征点  
        ///// </summary>  
        //private void GetBlockReferencePoints(DBTrans tr, BlockReference blkRef, List<Point2d> points)
        //{
        //    BlockTableRecord blockDef = tr.GetObject(blkRef.BlockTableRecord, OpenMode.ForRead) as BlockTableRecord;
        //    if (blockDef == null) return;

        //    // 从BlockReference中提取变换矩阵  
        ...
        //        // 获取实体的边界框并应用变换  
        //        Extents3d ext = ent.GeometricExtents;
        //        Point3d minPt = ext.MinPoint.TransformBy(blockTransform);
        //        Point3d maxPt = ext.MaxPoint.TransformBy(blockTransform);
        ...
        //        {
        //            // 创建复合变换（当前块变换 * 嵌套块变换）  
        //            Matrix3d combinedTransform = nestedBlkRef.BlockTransform.PreMultiplyBy(blockTransform);

        //            // 直接使用新的变换矩阵替换嵌套块的变换  
        //            BlockReference clonedRef = (BlockReference)nestedBlkRef.Clone();
        //            clonedRef.BlockTransform = combinedTransform;

        //            GetBlockReferencePoints(tr, clonedRef, points);
        ...
        //        {
        //            // 直接变换圆心  
        //            Point3d center = circle.Center.TransformBy(blockTransform);

        //            // 计算变换后的半径（使用一个向量来确定缩放）  
        //            Point3d radiusPoint = circle.Center + new Vector3d(circle.Radius, 0, 0);
        //            Point3d transformedRadiusPoint = radiusPoint.TransformBy(blockTransform);
        //            double transformedRadius = center.DistanceTo(transformedRadiusPoint);

        //            // 添加圆周上的点  
        //            for (double angle = 0; angle < 2 * Math.PI; angle += Math.PI / 4)
        //            {
        //                double x = center.X + transformedRadius * Math.Cos(angle);
        //                double y = center.Y + transformedRadius * Math.Sin(angle);
        ...
        //            {
        //                Point3d vertex = new Point3d(poly.GetPoint2dAt(i).X, poly.GetPoint2dAt(i).Y, 0);
        //                vertex = vertex.TransformBy(blockTransform);
        ...
        //        {
        //            // 变换中心点  
        //            Point3d center = arc.Center.TransformBy(blockTransform);

        //            // 计算变换后的半径  
        //            Point3d radiusPoint = arc.Center + new Vector3d(arc.Radius, 0, 0);
        //            Point3d transformedRadiusPoint = radiusPoint.TransformBy(blockTransform);
        //            double transformedRadius = center.DistanceTo(transformedRadiusPoint);
        ...
        //                Point3d transformed = original.TransformBy(blockTransform);
        //                points.Add(new Point2d(transformed.X, transformed.Y));
        //            }
        //        }
        //        else if (ent is Line line)
        //        {
        //            Point3d start = line.StartPoint.TransformBy(blockTransform);
        //            Point3d end = line.EndPoint.TransformBy(blockTransform);

        //            points.Add(new Point2d(start.X, start.Y));
        //            points.Add(new Point2d(end.X, end.Y));
        //        }
        //    }
        //}

        ///// <summary>  
        ///// 计算点集的凸包（Graham Scan算法）  
        ///// </summary>  
        ...
        //    // 计算其他点相对于最低点的极角  
        //    List<PointWithAngle> sortedPoints = new List<PointWithAngle>();
        //    foreach (Point2d point in points)
        //    {
        //        if (point.Equals(lowestPoint)) continue;

        //        double dx = point.X - lowestPoint.X;
        //        double dy = point.Y - lowestPoint.Y;
        //        double angle = Math.Atan2(dy, dx);

        //        sortedPoints.Add(new PointWithAngle { Point = point, Angle = angle });
        //    }

        //    // 按极角排序  
        //    sortedPoints.Sort((a, b) => a.Angle.CompareTo(b.Angle));

        //    // 构建凸包  
        //    List<Point2d> hull = new List<Point2d>();
        //    hull.Add(lowestPoint);

        //    foreach (PointWithAngle pa in sortedPoints)
        //    {
        //        while (hull.Count >= 2 && !IsLeftTurn(hull[hull.Count - 2], hull[hull.Count - 1], pa.Point))
        //        {
        ...
        //    if (hull.Count >= 3)
        //    {
        //        while (!IsLeftTurn(hull[hull.Count - 2], hull[hull.Count - 1], hull[0]))
        //        {
        //            hull.RemoveAt(hull.Count - 1);
        //            if (hull.Count < 3) break;
        //        }
        //    }
        ...
        ///// 判断从a->b->c是否为左转（叉积>0）  
        ///// </summary>  
        //private bool IsLeftTurn(Point2d a, Point2d b, Point2d c)
        //{
        ...
        ///* === 参数区 === */
        //// 偏移距离：正＝向外，负＝向内  
        //const double OFFSET_DIST = 10.0;
        //// 最大递归深度（爆炸块/组合体时使用）  
        //const int MAX_DEPTH = 20;
        ///* === 命令入口 === */
        //[CommandMethod(nameof(GenerateOutline))]
        ...
        //        // 防循环集合  
        //        var processedBlkIds = new HashSet<ObjectId>();
        ...
        //            ed.WriteMessage($"\n>>> 正在处理 {ent.GetType().Name}...");

        //            // 2-1) 递归收集此图元能获得的 *闭合* 曲线  
        //            var rawCurves = new List<Curve>();
        //            CollectClosedCurves(ent, tr, rawCurves, processedBlkIds, 0);
        ...
        //            // 2-2) 将所有闭合曲线变成 Region，并做 Union  
        //            Autodesk.AutoCAD.DatabaseServices.Region mergedRegion = BuildUnionRegion(rawCurves, ed);

        //            // 释放曲线  
        //            rawCurves.ForEach(c => c.Dispose());
        ...
        //            // 2-3) 取 Region 边界对象  
        //            var borders = new DBObjectCollection();
        //            mergedRegion.GetBoundingBoxEx();
        //            mergedRegion.Dispose();                               // Region 用完即弃  

        //            foreach (DBObject obj in borders)
        //            {
        //                // Region 的边界必为 Curve  
        ...
        //                // 转多段线，便于计算面积及偏移  
        //                var pl = CurveToPline(curve);
        //                curve.Dispose();

        //                if (pl == null) continue;

        //                // 面积 > 0 = 顺时针(外圈)，<0=孔洞；只偏移外圈  
        //                if (GetAreaSafe(pl) > 0)
        //                {
        //                    foreach (Entity off in pl.GetOffsetCurves(OFFSET_DIST))
        //                    {
        //                        off.ColorIndex = 1;             // 红色区分  
        //                        ms.AppendEntity(off);
        ...
        //            // 释放边界临时对象  
        //            foreach (DBObject o in borders) o.Dispose();
        //        }
        ...
        //}
        ///* === 辅助函数 === */
        //// 递归提取闭合曲线，跳过未闭合曲线可避免生成伪 Region  
        //private void CollectClosedCurves(Entity e, Transaction tr,
        //                                 List<Curve> dst,
        //                                 HashSet<ObjectId> doneBlk,
        //                                 int depth)
        //{
        ...
        //    switch (e)
        //    {
        //        case Curve cv when cv.Closed:
        //            dst.Add((Curve)cv.Clone());           // 克隆放入结果  
        //            return;

        //        case BlockReference br:
        //            if (!doneBlk.Add(br.BlockTableRecord)) return;   // 已处理则退出  

        //            using (var set = new DBObjectCollection())
        //            {
        //                br.Explode(set);
        //                foreach (DBObject o in set)
        //                {
        //                    if (o is Entity subE)
        //                        CollectClosedCurves(subE, tr, dst, doneBlk, depth + 1);
        //                    o.Dispose();
        //                }
        //            }
        //            doneBlk.Remove(br.BlockTableRecord);
        //            return;

        //        // 可爆炸的三维体 / Region / Surface  
        ...
        //// 把若干闭合曲线构造成并集 Region  
        //private Autodesk.AutoCAD.DatabaseServices.Region BuildUnionRegion(List<Curve> curves, Editor ed)
        //{
        //    Autodesk.AutoCAD.DatabaseServices.Region union = null;
        ...
        //            set.Add((DBObject)c.Clone());

        //            var regs = Autodesk.AutoCAD.DatabaseServices.Region.CreateFromCurves(set);
        //            foreach (DBObject o in set) o.Dispose();

        //            if (regs.Count == 0) continue;

        //            var reg = (Autodesk.AutoCAD.DatabaseServices.Region)regs[0];
        ...
        //            else
        //            {
        //                union.BooleanOperation(BooleanOperationType.BoolUnite, reg);
        //                reg.Dispose();
        //            }
        //        }
        //        catch (System.Exception ex)
        //        {
        //            ed.WriteMessage($"\nRegion 构造异常: {ex.Message}");
        //        }
        //    }
        //    return union;
        //}
        ...
        //// 把任意 Curve 精简离散为多段线  
        //private Polyline CurveToPline(Curve cv)
        //{
        //    // 已经是多段线  
        //    if (cv is Polyline pl0) return (Polyline)pl0.Clone();

        //    // 普通曲线→按弧长采样  
        //    int segs = 128;
        //    try
        //    {
        //        // 依据长度动态细分（最小 32，最大 512）  
        //        double len = cv.GetDistanceAtParameter(cv.EndParam) -
        //                     cv.GetDistanceAtParameter(cv.StartParam);
        //        segs = Math.Max(32, Math.Min(512, (int)(len / 2)));
        //    }
        //    catch { }

        //    var pl = new Polyline();
        //    double start = cv.StartParam, end = cv.EndParam;
        //    double step = (end - start) / segs;

        //    for (int i = 0; i <= segs; i++)
        //    {
        //        double t = start + i * step;
        //        Point3d pt = cv.GetPointAtParameter(t);
        ...
        //// 安全获取面积（Polyline2d/3d 时返回 0）  
        //private double GetAreaSafe(Polyline pl)
        //{
        ...
        //// 采样点数（越大越圆滑，128已经大于一般精度需求）  
        //const int OutlineSampleNum = 128;

        //[CommandMethod("FASTOUTLINE")]
        //public void FastOutline()
        //{
        //    // 获取当前文档相关环境  
        //    Document doc = Application.DocumentManager.MdiActiveDocument;
        //    Editor ed = doc.Editor;
        ...
        //            // 1. 获取对象的包围盒  
        //            Extents3d? ext = GetEntityExtents(ent);
        ...
        //            double step = 2 * Math.PI / OutlineSampleNum;

        //            for (int i = 0; i < OutlineSampleNum; i++)
        //            {
        //                double ang = i * step;
        //                Vector2d dir = new Vector2d(Math.Cos(ang), Math.Sin(ang));
        //                Point2d rayStart = new Point2d(center.X, center.Y);

        //                // 足够长的射线路径  
        //                Point2d rayEnd = new Point2d( center.X + dir.X * 1e4,center.Y + dir.Y * 1e4);

        //                // 取遍所有包围盒边界上的交点，即图元"外廓"上的点  
        //                Point2d? pt = FindEntityOuterBoundaryPoint(ent, rayStart, rayEnd, tr);

        //                if (pt != null)
        //                    edgePoints.Add(pt.Value);
        //                // 如果没找到，采样丢弃，保证图元缺角时结果不乱  
        //            }

        //            if (edgePoints.Count < 3) continue; // 至少需要三点生成多段线  

        //            // 4. 用这些点生成一条闭合多段线  
        //            Polyline outlinePL = new Polyline();
        //            for (int j = 0; j < edgePoints.Count; j++)
        //            {
        //                outlinePL.AddVertexAt(j, edgePoints[j], 0, 0, 0);
        //            }
        //            outlinePL.Closed = true;

        //            // 5. 对“包络多段线”外扩（偏移）  
        //            DBObjectCollection offsets = outlinePL.GetOffsetCurves(OffsetDistance);
        //            foreach (Entity off in offsets)
        //            {
        //                off.ColorIndex = 1; // 红色  
        //                ms.AppendEntity(off);
        //                tr.AddNewlyCreatedDBObject(off, true);
        ...
        ///// <summary>
        ///// 对实体射线采样，找到实体边界上最远交点（近似其外部轮廓采样）  
        ///// </summary>
        ///// <param name="ent">实体对象</param>
        ///// <param name="rayStart">开始点</param>
        ///// <param name="rayEnd">结束点</param>
        ///// <param name="tr">事务</param>
        ///// <returns></returns>
        //private Point2d? FindEntityOuterBoundaryPoint(Entity ent, Point2d rayStart, Point2d rayEnd, Transaction tr)
        //{
        //    // 说明：本函数用来找到实体 ent 沿着一条射线（rayStart→rayEnd）上所有交点中距离 rayStart 最远的一个点，  
        //    // 这个点一般位于实体的外边界，用于采样实体外轮廓。  

        //    // 创建一个列表用于收集实体的所有曲线部分（方便后续计算交点）  
        //    List<Curve> curves = new List<Curve>();

        //    // 调用先前定义的递归方法，把当前实体及其子块的所有曲线收集到 curves 集合中  
        //    CollectAllCurves(ent, tr, curves);

        //    // 初始化一个可空二维点，用于存储当前找到的最远交点  
        //    Point2d? bestPt = null;

        //    // 初始化最大距离为最小值，目的是找到距离起点最远的点  
        //    double maxDist = double.MinValue;

        //    // 遍历 curves 集合中每条曲线，寻找与射线的交点  
        ...
        //            // 求曲线（cv）与从 rayStart 到 rayEnd 的直线的交点  
        //            cv.IntersectWith(
        //                new Line(
        //                    // 射线起点3D坐标 
        //                    new Point3d(rayStart.X, rayStart.Y, 0),
        //                    // 射线终点3D坐标  
        //                    new Point3d(rayEnd.X, rayEnd.Y, 0)      
        //                ),
        //                // 指定求两端都在双方曲线上的交点 
        ...
        //            {
        //                // 计算二维射线起点到交点的距离的平方（避免开根号，效率更高）  
        //                double dist = (p.X - rayStart.X) * (p.X - rayStart.X) +
        //                              (p.Y - rayStart.Y) * (p.Y - rayStart.Y);
        ...
        //                {
        //                    maxDist = dist;             // 更新最大距离  
        //                    bestPt = new Point2d(p.X, p.Y);  // 记录该距离对应的点  
        //                }
        //            }
        //        }
        //        catch
        //        {
        //            // 如果求交过程中出错，捕获异常但不处理，继续处理其它曲线  
        //        }
        //    }

        //    // 释放之前克隆创建的所有曲线资源，避免内存泄漏  
        //    foreach (var cc in curves) cc.Dispose();

        //    // 返回当前射线方向上距离起点最远的交点（如果无交点返回null）  
        //    return bestPt;
        //}
        ...
        ///// 递归收集所有曲线（包括块参照内的）  
        ///// </summary>
        ///// <param name="ent">实体对像</param>
        ///// <param name="tr">事务</param>
        ///// <param name="curveList">曲线列队</param>
        ///// <param name="depth"></param>
        //private void CollectAllCurves(Entity ent, Transaction tr, List<Curve> curveList, int depth = 0)
        //{
        //    // 递归深度限制，防止过深递归导致栈溢出  
        //    if (depth > 10) return;
        ...
        //    // 如果当前实体是曲线（Curve）类型  
        //    if (ent is Curve c)
        //        // 克隆当前曲线并加入结果列表，避免修改原数据  
        //        curveList.Add((Curve)c.Clone());      

        //    // 如果当前实体是块参照（BlockReference），需要爆炸提取内部子实体  
        //    else if (ent is BlockReference br)
        //    {
        //        // 使用using保证DBObjectCollection会被及时释放  
        //        using (var subObjs = new DBObjectCollection())
        //        {
        //            // 爆炸块参照，将子对象加入集合  
        //            br.Explode(subObjs);
        //            // 遍历爆炸得到的所有对象  
        //            foreach (DBObject o in subObjs)   
        //            {
        //                // 如果是实体，递归调用本函数收集其中的曲线，深度+1  
        //                if (o is Entity se)
        //                    CollectAllCurves(se, tr, curveList, depth + 1);
        ...
        //// 获取实体包围盒（自动处理XData无效等情况）  
        //private Extents3d? GetEntityExtents(Entity ent)
        //{
        //    try
        //    {
        ...
        ///// 外扩（偏移）距离，单位为CAD当前单位   
        ///// </summary>  
        //const double OffsetDistance = 10.0; // 偏移距离  

        ///// <summary>  
        ///// 一圈采样点数，越大精度越高，128通常足够  
        ///// </summary>  
        //const int OutlineSampleNum = 128; // 采样点数  

        //[CommandMethod("FASTOUTLINE")]
        //public void FastOutline()
        //{
        //    // 获取当前文档、数据库和命令窗口  
        //    Document doc = Application.DocumentManager.MdiActiveDocument;            // 文档对象  
        //    Database db = doc.Database;                                              // 数据库对象  
        //    Editor ed = doc.Editor;                                                  // 命令行输出对象  

        //    // 让用户选择一个或多个图元  
        //    PromptSelectionOptions selOpt = new PromptSelectionOptions();            // 选择参数  
        //    selOpt.MessageForAdding = "\n请选择要生成外轮廓的图元 : ";               // 消息提示  
        //    PromptSelectionResult selRes = ed.GetSelection(selOpt);           // 执行选择  
        //    if (selRes.Status != PromptStatus.OK) return;                            // 用户取消则退出  
        //    // 开启事务 
        //    using (var tr = new DBTrans())
        //    {
        //        // 获取模型空间以便添加新图元  
        //        BlockTable bt = (BlockTable)tr.GetObject(db.BlockTableId, OpenMode.ForRead);    // 打开块表  
        //        BlockTableRecord ms = (BlockTableRecord)tr.GetObject(bt[BlockTableRecord.ModelSpace], OpenMode.ForWrite); // 打开模型空间  

        //        // -------【处理整个选集为一个包络轮廓】-------  

        //        // 1. 收集所有曲线  
        //        List<Curve> allCurves = new List<Curve>();                                          // 存储所有曲线的列表  
        //        foreach (SelectedObject selObj in selRes.Value)                                     // 遍历所选对象  
        //        {
        //            if (selObj == null) continue;                                                   // 防空判断  
        //            Entity ent = tr.GetObject(selObj.ObjectId, OpenMode.ForRead) as Entity; // 获取实体  
        //            if (ent == null) continue;                                                      // 非实体跳过  
        //            CollectAllCurves(ent, tr, allCurves, 0);                          // 收集此实体及块内曲线  
        //        }
        //        if (allCurves.Count == 0) return;                                                   // 没有曲线则退出  

        //        // 2. 计算所有选中对象的联合包围盒中心点                                          
        //        Extents3d ext = GetBoundingExtentsFromCurves(allCurves);                     // 根据曲线计算包围盒  
        //        Point3d min = ext.MinPoint;                                                        // 最小点  
        //        Point3d max = ext.MaxPoint;                                                        // 最大点  
        //        Point3d center = new Point3d((min.X + max.X) / 2, (min.Y + max.Y) / 2, 0); // 中心点  

        //        // 3. 以中心点顺时针扫描，采样轮廓极点  
        //        List<Point2d> edgePoints = new List<Point2d>();                                    // 存储极点  
        //        //double step = 2 * Math.PI / OutlineSampleNum;                                      // 每步角度增量  
        //        double step = 2 * Math.PI / 365;                                      // 每步角度增量  

        //        ////for (int i = 0; i < OutlineSampleNum; i++)                                         // 遍历每个角度  
        //        for (int i = 0; i < 365; i++)                                         // 遍历每个角度  
        //        {
        //            double ang = i * step;                                                         // 当前角度  
        //            Vector2d dir = new Vector2d(Math.Cos(ang), Math.Sin(ang));           // 方向向量  
        //            Point2d rayStart = new Point2d(center.X, center.Y);                       // 射线起点  
        //                                                                                      // 射线终点足够远  
        //            Point2d rayEnd = new Point2d(center.X + dir.X * 1e4, center.Y + dir.Y * 1e4); // 射线终点  
        //                                                                                          // 查找此方向上所有曲线的最外端交点  
        //            Point2d? pt = FindOuterBoundaryPoint(allCurves, rayStart, rayEnd);       // 求交点  
        //            if (pt != null)
        //                edgePoints.Add(pt.Value);                                             // 记录极点  
        //                                                                                      // 没找到则跳过，缺口自动补齐  
        //        }

        //        if (edgePoints.Count < 3)                                                          // 点数不足  
        //        {
        //            ed.WriteMessage("\n提取点数不足，无法生成多段线！\n");                  // 警告  
        //            foreach (var cc in allCurves) cc.Dispose();                               // 释放曲线  
        //            return;                                                                        // 退出  
        //        }

        //        // 4. 用极点生成一条闭合多段线  
        //        Polyline outlinePL = new Polyline();                                              // 新建多段线  
        //        for (int j = 0; j < edgePoints.Count; j++)                                        // 遍历所有点  
        //        {
        //            outlinePL.AddVertexAt(j, edgePoints[j], 0, 0, 0);        // 添加顶点  
        //        }
        //        outlinePL.Closed = true;                                                          // 闭合多段线  
        //        isNotLayer(tr, "SB", 1); // 判断是否存在图层,如果没有就创建一个

        //        outlinePL.Layer = "SB"; // 设置图层
        //        tr.CurrentSpace.AddEntity(outlinePL);
        //        //tr.AddNewlyCreatedDBObject(outlinePL, true);
        //        //// 5. 对多段线做正向偏移  
        //        //DBObjectCollection offsets = outlinePL.GetOffsetCurves(OffsetDistance); // 偏移结果  

        //        //foreach (Entity off in offsets)                                                   // 遍历所有偏移曲线  
        //        //{
        //        //    off.ColorIndex = 1; // 红色区分  
        //        //    ms.AppendEntity(off);                                                  // 加入模型空间  
        //        //    tr.AddNewlyCreatedDBObject(off, true);                               // 注册到事务  
        //        //}                                                                         

        //        //// 6. 删除临时对象                                                        
        //        //outlinePL.Dispose();                                                             // 释放多段线  
        //        //foreach (var cc in allCurves) cc.Dispose();                                 // 释放曲线  
        ...
        //        // -----[若需要每个对象生成各自包络，把foreach改为单对象版即可]-----  
        //    }
        //}

        ///// <summary>  
        ///// ★ 收集实体（含块内容）所有曲线，递归爆炸，防StackOverflow故做深度限制  
        ///// </summary>  
        ///// <param name="ent">选择的实体</param>  
        ///// <param name="tr">开启事务</param>  
        ///// <param name="curveList">曲线列队</param>  
        ///// <param name="depth">深度</param>  
        //private void CollectAllCurves(Entity ent, Transaction tr, List<Curve> curveList, int depth = 0)
        //{
        //    if (depth > 10) return; // 深度超过限制则返回  

        //    if (ent is Curve c)      // 如果是曲线  
        //        curveList.Add((Curve)c.Clone()); // 克隆后加入列表  

        //    else if (ent is BlockReference br) // 如果是块参照  
        //    {
        //        using (var subObjs = new DBObjectCollection()) // 爆炸结果容器  
        //        {
        //            br.Explode(subObjs);                      // 爆炸块参照  
        //            foreach (DBObject o in subObjs)           // 遍历子对象  
        //            {
        //                if (o is Entity se)                   // 如果是实体  
        //                    CollectAllCurves(se, tr, curveList, depth + 1); // 递归收集  
        //                o.Dispose();                          // 释放临时对象  
        //            }
        //        }
        //    }
        //    // 如有表面/面体Region等可以继续递归爆炸配合  
        //}

        ///// <summary>  
        ///// ★ 获取一组曲线的整个联合包围盒  
        ///// </summary>  
        ///// <param name="curves">曲线列队</param>  
        ///// <returns>联合包围盒</returns>  
        //private Extents3d GetBoundingExtentsFromCurves(List<Curve> curves)
        //{
        //    bool first = true;             // 首次标志  
        //    Extents3d ext = new Extents3d(); // 包围盒变量  

        //    foreach (Curve c in curves)    // 遍历所有曲线  
        //    {
        //        try
        //        {
        //            if (!c.Bounds.HasValue) continue; // 没有包围盒则跳过  
        //            if (first)
        //            {
        //                ext = c.Bounds.Value;          // 首次赋值  
        //                first = false;                 // 标记已赋值  
        //            }
        //            else
        //            {
        //                ext.AddExtents(c.Bounds.Value); // 合并包围盒  
        //            }
        //        }
        //        catch { }
        //    }
        ...
        ///// <summary>  
        ///// ★ 在所有曲线上查找给定射线上最远交点（外轮廓点）  
        ///// </summary>  
        ///// <param name="curves">曲线列队</param>  
        ///// <param name="rayStart">射线起点</param>  
        ///// <param name="rayEnd">射线终点</param>  
        ///// <returns>最远交点（二维）或null</returns>  
        //private Point2d? FindOuterBoundaryPoint(List<Curve> curves, Point2d rayStart, Point2d rayEnd)
        //{
        //    Point2d? bestPt = null;               // 最佳点初始为null  
        //    double maxDist = double.MinValue;     // 最大距离初始为最小值  

        //    foreach (Curve cv in curves)          // 遍历所有曲线  
        //    {
        //        try
        //        {
        //            Point3dCollection pts = new Point3dCollection(); // 存储交点  
        //            cv.IntersectWith(                                  // 计算交点  
        //                new Line(                                     // 射线实体  
        //                    new Point3d(rayStart.X, rayStart.Y, 0),  // 起点3D  
        //                    new Point3d(rayEnd.X, rayEnd.Y, 0)       // 终点3D  
        //                ),
        //                Intersect.OnBothOperands,                     // 仅取实际交点  
        //                pts,                                          // 存储结果  
        //                IntPtr.Zero,
        //                IntPtr.Zero
        //            );
        //            foreach (Point3d p in pts)                       // 遍历每个交点  
        //            {
        //                double dist =                             // 计算距离平方  
        //                    (p.X - rayStart.X) * (p.X - rayStart.X) +
        //                    (p.Y - rayStart.Y) * (p.Y - rayStart.Y);
        //                if (dist > maxDist)                       // 如果更远  
        //                {
        //                    maxDist = dist;                       // 更新最大距离  
        //                    bestPt = new Point2d(p.X, p.Y);       // 更新最佳点  
        //                }
        ...
        //        catch { } // 出错则跳过此曲线  
        //    }
        //    return bestPt; // 返回最远交点或null  
        //}
        #endregion


        private bool isNotLayer(DBTrans tr, string layerName, int layerColor)
        {
            if (!tr.LayerTable.Has(layerName))
            {
        ...
        private bool isNotStyle(DBTrans tr, string textStyleStr)
        {
            if (!tr.TextStyleTable.Has(textStyleStr))
            {
                TextStyle newTtextStyle = new TextStyle()
                {
                    FileName = "gbenor.shx",// 字体文件
                    BigFontFileName = "gbcbig.shx",// 大字体文件
                    XScale = 1, // 字体比例
                };
                newTtextStyle.StyleName = textStyleStr;
            }
            return true;
        }
        ///////////////////////////////////////////////////////////////////
        ...
        private void RemoveDictionaryEntry()
        {
            try
            {
                ...
                // 删除指定的字典项  
                if (nod.Contains("ACAD_DGNLINESTYLECOMP"))
                {
                    nod.Remove("ACAD_DGNLINESTYLECOMP");
                    tr.Commit();
                }
            }
            catch (System.Exception ex)
            {
                Env.Editor.WriteMessage($"\nError removing dictionary entry: {ex.Message}");
            }
        }
        ...
        /// 顺序执行命令  
        /// </summary>  
        private void ExecuteCommandSequentially()
        {
            try
            {
                // 快速保存  
                Env.Editor.WriteMessage("_.QSAVE");

                // 执行清理命令 (使用两个回车跳过确认)  
                Env.Editor.WriteMessage("_PURGE", "ALL", "", "N");

                // 重置比例列表  
                Env.Editor.WriteMessage("-SCALELISTEDIT", "RESET", "Y", "E");
            ...
            {
                Env.Editor.WriteMessage($"\nError executing commands: {ex.Message}");
            }
        }
        #endregion
    }
