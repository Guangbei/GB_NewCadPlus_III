...
using System.Web.Script.Serialization; // JavaScriptSerializer（用于 JSON 序列化/反序列化，兼容 .NET Framework 4.8）
...
namespace GB_NewCadPlus_III
{
    ...
    {
        public string Original { get; set; } = ""; // 原图层名称
        public string Dic { get; set; } = "";      // 解释后的图层名称
    }
    ...
    /// </summary>
    public class LayerDictionaryEntry
    {
...
CREATE TABLE IF NOT EXISTS `layer_dictionary` (
  ...
  INDEX (`major`)
        ...
        /// </summary>
        public static async Task<List<LayerDictionaryEntry>> GetLayerDictionaryByUsernameAsync(this DatabaseManager db, string username)
        {
            if (db == null) return new List<LayerDictionaryEntry>(); // 参数保护
            const string sql = @"
SELECT
  id AS Id,
  seq AS Seq,
...
ORDER BY seq;"; // 查询 SQL（注意选取 mappings_json 列）
            ...
            {
                using var conn = db.GetConnection(); // 获取连接
                var rows = await conn.QueryAsync<LayerDictionaryEntry>(sql, new { Username = username }).ConfigureAwait(false); // 执行查询
            ...
            catch (Exception ex)
            {
                ...
                return new List<LayerDictionaryEntry>(); // 返回空列表
            }
        }
        ...
        /// </summary>
        public static async Task<List<LayerDictionaryEntry>> GetStandardLayerDictionaryByMajorAsync(this DatabaseManager db, string major)
        {
            if (db == null) return new List<LayerDictionaryEntry>(); // 参数保护
            const string sql = @"
SELECT
...
ORDER BY seq;"; // 管理员发布且按 major 可选过滤
            ...
            {
                using var conn = db.GetConnection(); // 获取连接
                var rows = await conn.QueryAsync<LayerDictionaryEntry>(sql, new { Major = major }).ConfigureAwait(false); // 执行查询
        ...
        /// </summary>
        public static async Task<bool> SaveLayerDictionaryForUserAsync(this DatabaseManager db, string username, List<LayerDictionaryEntry> entries)
        {
            if (db == null) return false; // 参数保护
            try
            {
